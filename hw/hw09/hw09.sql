...
CREATE TABLE by_parent_height AS
  SELECT a.child from parents as a, dogs as b where a.parent = b.name
  ORDER by height DESC;

-- Filling out this helper table is optional
CREATE TABLE siblings AS
  SELECT a.child as first, b.child as second from parents as a, parents as b where a.parent = b.parent
  and a.child < b.child;

-- Sentences about siblings that are the same size
CREATE TABLE siblings_with_same_size AS 
...
CREATE TABLE sentences AS
  SELECT first || ' and ' || second || ' are ' || size || ' siblings' from siblings_with_same_size;