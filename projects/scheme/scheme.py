...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        #use lambda functions to curry scheme_eval into a one argument function
...
def eval_all(expressions, env):
    ...
    while expressions != nil:
        curr_num = scheme_eval(expressions.first, env, expressions.rest is nil)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): 
        ...
        # use .rest.first to obtain the inner nested pair instance
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        eval_value = scheme_eval(expressions.first, env, expressions.rest is nil)
        if is_false_primitive(eval_value) or expressions.rest == nil:
            return eval_value
        expressions = expressions.rest
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    while expressions != nil:
        eval_value = scheme_eval(expressions.first, env, expressions.rest is nil)
        if is_true_primitive(eval_value) or expressions.rest is nil:
            return eval_value
        expressions = expressions.rest
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    # bindings -> ((a 1) (b 2))
    # names, values = nil, nil
    # at bindings.first = (a 1)
    # bindings.first.first = a, bindings.first.rest.first = 1
    # names = Pair(a, names=nil), values = Pair(1, values=nil)
    # bindings = bindings.rest = (b 2)
    # bindings.first.first = b, bindings.first.rest.first = 2
    # names = Pair(b, names=Pair(a, bil)) and values = Pair(2, values=Pair(1, nil))
    # names -> (a b)
    # values -> (1 2)
    while bindings != nil:
...
def do_define_macro(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    validate_form(expressions, 2)
    target = expressions.first  #(f x)
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        validate_formals(target.rest) #(x)
        #target.rest = (x), expressions.rest = (1 2)
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a dynamically-scoped call evaluated in current environment ENV.
        Compared to LambdaProcedure which has an instance attribute known as self.env, 
        MuProcedures have no self.env and upon being called, will create a frame whose parent
        is the frame in which it was called on, such that the mu function has access to attributes
        in the frame it was called in (dynamic scope)"""
